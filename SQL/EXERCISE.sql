...
--new query pedndng
SELECT NAME,CITY,STATES 
    FROM CLIENT_MASTER WHERE STATES NOT IN ('Maharashtra');
SELECT *
    FROM SALES_ORDER_MASTER WHERE TO_CHAR(ORDERDATE,'MON') = 'JUN';
SELECT TO_CHAR (ORDERDATE,'DAY')
    AS DAY FROM SALES_ORDER_MASTER;
...
SELECT ORDERNO,TO_CHAR(DELYDATE,'DD-MONTH') 
    AS DELY_DAY FROM SALES_ORDER_MASTER;
